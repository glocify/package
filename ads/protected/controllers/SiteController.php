...
class SiteController extends CController
{
	...
	public function actionIndex()
	{
		...
		$this->render('index',array('Userdata'=>$Userdata, 'pages'=>$pages));
	...
	{		
		...
		$this->renderPartial('//layouts/header');
		$this->render('listing',array('Userdata'=>$Userdata, 'pages'=>$pages)); 
		$this->renderPartial('//layouts/footer');
	}
	...
	{	
		...
		if(isset($_POST['submit'])) {
				...
				if(count($pagedta) < 4){
					$model=new Jobs;					
					$model->send_to_user=$_POST['sent_to_user'];
					...
					$model->how_many_freenlancer=$_POST['how_many_freelancer'];
					...
					if($model->save()):
						...
						->select('*')
						->from('cads_Users')
						->where('id =:id',array(':id' => $_POST['sent_to_user']))
						->queryRow();
										...
										<table width="100%" cellspacing="8" cellpadding="8">
											<tr>
												<td align="left" valign="top">
													...
													<p> How many freelancers required  : '.$_POST['how_many_freelancer'].' </p>
						...
						// Additional headers
						$headers .= 'To: '.$_POST['firstname'].' <'.$_POST['email_address'].'>' . "\r\n";
						$headers .= 'From: Money Media Solutions <upworksanjeev@gmail.com>' . "\r\n";
						
						// Mail it
						if(mail($to, $subject, $message, $headers)){
							Yii::app()->user->setFlash('success','Your Job has been posted successfully.');
						...
						Yii::app()->user->setFlash('error','Something went wrong. Please contact with adminstration.');
						$this->redirect(array('Site/index'));
					endif;
					
				}else{	
					$redirect_url = $_POST['redirect_url'];
					Yii::app()->user->setFlash('error','Your already sent job to 4 profiles.');
					$this->redirect($redirect_url);
				}
		}else{
			$this->redirect(array('Site/index'));
